...
case class RawDomain(
  ...
  enums: ListMap[String, RawEnumData] = ListMap.empty,
  labels: Labels = Map.empty,
  errors: Seq[String] = Seq.empty,
) extends ModelValidator {

  def withComponentSeq(componentSeq: Seq[RawComponentData]): RawDomain = {
  ...
  def withComponents(componentSeq: RawComponentData*) = withComponentSeq(componentSeq)

  def withDataElementSeq(seq: Seq[RawDataElement]) = {
  ...
  def withDataElements(seq: RawDataElement*) = withDataElementSeq(seq)

  def withEnumSeq(seq: Seq[RawEnumData]) =
    copy(enums = ListMap.from(seq.map(e => e.id -> e)))

  def withEnums(seq: RawEnumData*) = withEnumSeq(seq)

  def withLabels(seq: Label*) = copy(labels = labels ++ seq.map(a => a.key -> a.value))

  def build(replicateAttr: Boolean = true): Either[Seq[String], RawDomain] = {
